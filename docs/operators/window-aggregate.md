...
An existing LTZ limitation is tracked in
[#119](https://github.com/datafusion-contrib/StreamFusion/issues/119): standalone TVFs in an
admitted nonzero fixed-offset session zone currently expose epoch-grid start/end values rather
than local boundaries. The zone gate below does not prevent this output error. UTC LTZ TVFs
retain their existing parity coverage.
