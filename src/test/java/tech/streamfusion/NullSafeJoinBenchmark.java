...
class NullSafeJoinBenchmark {
  private static final long ROWS = Long.getLong("join.rows", 1_000_000L);
  ...
  private static final String SQL = "INSERT INTO sink SELECT a.v FROM A a JOIN B b "
      + "ON a.k IS NOT DISTINCT FROM b.k";

  @Test
  void nullSafeJoin() throws Exception {
    ...
    System.out.printf(Locale.ROOT,
        "[null-safe-join] rows=%d Flink=%.6fs Native=%.6fs ratio=%.3fx host_trials=%s native_trials=%s%n",
  ...
  private static TableEnvironment environment() {
    ...
    table.createTemporaryView("B", env.fromSequence(0, 1023)
        .map(i -> Row.of(i == 1023 ? null : i))
        .returns(Types.ROW_NAMED(new String[] {"k"}, Types.LONG)));
    table.executeSql("CREATE TABLE sink (v BIGINT) WITH ('connector' = 'blackhole')");
    return table;
  }
}
