...
class MixedWindowAvgBenchmark {
  ...
  private static final String SQL = "INSERT INTO sink SELECT k, COUNT(*), AVG(v), SUM(v), "
  ...
  void mixedWindowAvg(String phase) throws Exception {
    ...
    if (!plan.contains(phase.equals("TWO_PHASE")
        ? "NativeColumnarGlobalWindowAggregate" : "NativeColumnarWindowAggregate") || !plan.contains("RowDataToArrow")
        || !plan.contains("ArrowToRowData")) {
      throw new IllegalStateException("Expected native window aggregate and both transposes: " + plan);
    }
    ...
    System.out.printf(Locale.ROOT,
        "[mixed-window-avg] phase=%s rows=%d Flink=%.6fs Native=%.6fs ratio=%.3fx host_trials=%s native_trials=%s%n",
        phase, ROWS, host, nativeTime, host / nativeTime, Arrays.toString(times[0]), Arrays.toString(times[1]));
  }
  ...
  private static TableEnvironment environment(String phase) {
    ...
    table.createTemporaryView("src", input, Schema.newBuilder()
        .column("k", DataTypes.BIGINT()).column("v", DataTypes.BIGINT()).column("i", DataTypes.INT())
        .column("ts", DataTypes.BIGINT()).columnByMetadata("rt", DataTypes.TIMESTAMP_LTZ(3), "rowtime")
        .watermark("rt", "SOURCE_WATERMARK()").build());
    table.executeSql("CREATE TABLE sink (k BIGINT, c BIGINT, av BIGINT, s BIGINT, mn BIGINT, mx BIGINT, ai INT) "
