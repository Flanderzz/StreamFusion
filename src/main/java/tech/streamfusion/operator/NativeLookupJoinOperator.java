 ...
 * connector's real {@code LookupFunction}, the optional projection/filter on the dimension table,
 * the residual join condition, and LEFT null-padding — gathering the joined rows back into an Arrow
 * batch. Byte-identical to the host by construction, since the row-level core <em>is</em> the
 * host's code; keeping the operator's boundary Arrow lets the probe-side Calc/source stay in the
 * native island rather than the whole query falling back around the lookup.
 *
 ...
 *
 * <p>The pre-filter runs without streaming context, exactly as Flink's own async lookup runner
 * evaluates the same condition — a pre-filter needing the streaming context (current watermark /
 * record timestamp) fails loudly rather than silently diverging.
 */
public class NativeLookupJoinOperator extends FlinkStreamOperator<ArrowBatch>
  ...
  public void processElement(StreamRecord<ArrowBatch> element) throws Exception {
    ...
    try (VectorSchemaRoot root = element.getValue().root()) {
      ...
      for (int i = 0; i < rowCount; i++) {
        ...
        if (FlinkLookupCompat.preFilter(runner, probe)) {
