 ...
 * org.apache.flink.table.runtime.operators.window.tvf.unslicing.UnsliceWindowAggProcessor}-fed
 * {@code WindowTableFunctionOperator}. Each incoming batch is assigned to its window(s) natively
 * and emitted with {@code window_start}/{@code window_end}/{@code window_time} appended (rows
 * fanned out, one copy per window, for hopping/cumulative). It does no event-time buffering — the
 * downstream window join or aggregate does — so it forwards watermarks unchanged (the default
 * {@link AbstractStreamOperator} behavior); carrying Arrow lets it chain with the native window
 * operators without transposing.
 *
 ...
 */
public class NativeWindowTableFunctionOperator extends FlinkStreamOperator<ArrowBatch>
