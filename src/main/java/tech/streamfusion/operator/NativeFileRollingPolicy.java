 ...
 * (upstream hard-types it to {@code RowData}), with the bulk-format behavior fixed: a bulk-format
 * part file always rolls on checkpoint, because an in-progress bulk-format file has no footer and
 * cannot be resumed. Size checks see the drained stream position, which advances a flushed stripe
 * or row group at a time — the same granularity the host writer's size checks observe.
 */
public final class NativeFileRollingPolicy
  ...
  public NativeFileRollingPolicy(
  ...
  public boolean shouldRollOnEvent(
      PartFileInfo<String> partFileState, PartitionedArrowBatch element) throws IOException {
