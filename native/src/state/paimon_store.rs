...
impl PaimonKeepFirstStore {
    ...
    fn assemble(
        ...
        let max_parallelism = core.config.max_parallelism;
        let region = DirtyRegion::new(Self::value_fields(&payload_fields), Some(0), max_parallelism);
    ...
    pub(crate) fn stage(
        ...
        self.region.append_upserts(keys, values)
    }
    ...
    pub(crate) fn fire(
        ...
        if !fired_keys.is_empty() {
            ...
            self.region.append_upserts(&key_slices, values)?;
        }
        Ok(firing)
    }
